(function () {
  ...
  const avatar = person => `<span class="employee-avatar" aria-hidden="true">${person.foto ? `<img src="${e(person.foto)}" alt="" loading="lazy">` : e(initials(person.nama))}</span>`;
  ...
  function render() {
    ...
    $('employee-results').innerHTML = `<ul class="employee-grid" aria-label="Daftar karyawan">${results.map(person => `<li class="employee-card"><div class="employee-card-head">${avatar(person)}<div><h2><button class="employee-name-button" type="button" data-employee-action="detail" data-employee-id="${e(person.id)}" aria-haspopup="dialog" aria-controls="employee-detail-dialog" aria-label="Detail ${e(person.nama)} NIK ${e(person.nik)}">${e(person.nama)}</button></h2><span class="employee-nik">NIK ${e(person.nik)}</span></div></div><dl><dt>Jabatan</dt><dd>${e(person.jabatan)}</dd><dt>Golongan</dt><dd>${e(person.golongan || '—')}</dd><dt>No. HP</dt><dd>${e(person.noHp || '—')}</dd><dt>Kamar mess</dt><dd>${e(person.kamarMess || '—')}</dd></dl></li>`).join('')}</ul>`;
  }
  ...
  $('employee-detail-edit').addEventListener('click', async () => {
    ...
    if (!selectedEmployeeId || !window.Akses.canWrite() || button.disabled) return;
  ...
  async function editEmployee(id) {
    ...
    ['nik','nama','jabatan','golongan','noHp','kamarMess'].forEach(field => { $('employee-form').elements.namedItem(field).value = employee[field] || ''; });
